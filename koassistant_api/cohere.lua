...
function CohereHandler:buildRequestBody(message_history, config)
    local defaults = Defaults.ProviderDefaults.cohere
...
function CohereHandler:query(message_history, config)
    ...
    -- Cohere v2 API uses a similar format to OpenAI
    local request_body = {
        model = model,
        messages = {},
    }

    -- Add system message (preamble) from unified config
    -- Cohere v2 supports system role in messages array
    if config.system and config.system.text and config.system.text ~= "" then
        table.insert(request_body.messages, {
            role = "system",
            content = config.system.text,
