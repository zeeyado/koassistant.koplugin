...
function MistralHandler:buildRequestBody(message_history, config)
    local defaults = Defaults.ProviderDefaults.mistral
...
function MistralHandler:query(message_history, config)
    ...
    if config and config.features and config.features.debug then
        DebugUtils.print("Mistral Request Body:", request_body, config)
    ...
    if config and config.features and config.features.debug then
        DebugUtils.print("Mistral Raw Response:", table.concat(responseBody), config)
    end

    local success, response = self:handleApiResponse(success, code, responseBody, "Mistral")
    if not success then
        return response
    end
    ...
    if config and config.features and config.features.debug then
        DebugUtils.print("Mistral Parsed Response:", response, config)
    end
