...
local TogetherHandler = BaseHandler:new()
...
function TogetherHandler:buildRequestBody(message_history, config)
    local defaults = Defaults.ProviderDefaults.together
...
function TogetherHandler:query(message_history, config)
    ...
    if config and config.features and config.features.debug then
        DebugUtils.print("Together Request Body:", request_body, config)
    ...
    if config and config.features and config.features.debug then
        DebugUtils.print("Together Raw Response:", table.concat(responseBody), config)
    end

    local success, response = self:handleApiResponse(success, code, responseBody, "Together")
    if not success then
        return response
    end
    ...
    if config and config.features and config.features.debug then
        DebugUtils.print("Together Parsed Response:", response, config)
    end
