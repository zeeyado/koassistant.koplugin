...
local DoubaoHandler = BaseHandler:new()
...
function DoubaoHandler:buildRequestBody(message_history, config)
    local defaults = Defaults.ProviderDefaults.doubao
...
function DoubaoHandler:query(message_history, config)
    ...
    if config and config.features and config.features.debug then
        DebugUtils.print("Doubao Request Body:", request_body, config)
    ...
    if config and config.features and config.features.debug then
        DebugUtils.print("Doubao Raw Response:", table.concat(responseBody), config)
    end

    local success, response = self:handleApiResponse(success, code, responseBody, "Doubao")
    if not success then
        return response
    end
    ...
    if config and config.features and config.features.debug then
        DebugUtils.print("Doubao Parsed Response:", response, config)
    end

    local success, result = ResponseParser:parseResponse(response, "doubao")
    if not success then
        return "Error: " .. result
    end
