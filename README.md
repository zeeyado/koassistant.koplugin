...
- [Technical Features](#technical-features)
  ...
  - [AI Book Tools (Experimental)](#ai-book-tools-experimental): Let the AI (Gemini/Claude/OpenAI or Subscription/OpenRouter/Ollama, plus DeepSeek/Mistral/Groq/xAI/Fireworks/Qwen/Kimi/NVIDIA) search & read the open book on demand (off by default)
...
return {
    ...
    -- See apikeys.lua.sample for all 32 providers
}
```
...
- **Mentions**: unified chapter-navigable text matching. Opens showing every entry's mentions up to your position (**"To p. N"**), with comparison bars, counts, and category tags; a chapter picker at the top narrows the scope to any chapter or part (KOReader-style hierarchical TOC with expand/collapse, current chapter bolded). Includes an **"Entire document (to p. N)"** aggregate that scans from page 1 to your coverage boundary, plus **"Entire document"**, which reveals the whole book behind one confirmation. Chapters beyond the greater of X-Ray coverage and reading position are dimmed with tap-to-reveal spoiler protection, and a chapter that crosses your position is clipped at it ("up to p. N"). For complete X-Rays, all chapters are available with no spoiler gating. Books without a TOC fall back to page-range chunks. Excludes event-based categories (timeline, argument development) whose descriptive names produce misleading matches. Uses word-boundary matching against names and aliases (Arabic, Chinese, Japanese, Korean and Thai names match inside words, with or without Arabic vowel marks), the same matcher as Chapter Appearances and passive marking.
- **Chapter Appearances**: from any item's detail view, the book's whole table of contents as a tree (expand/collapse arrows on parent rows, the current chapter bold), with mention counts and comparison bars at every level. Counts use union semantics: all match spans from the item's name and aliases are collected, overlapping spans merged, and unique matches counted, matching KOReader's text search behavior. A hit that sits inside another entity's longer name counts only for that longer entity, so a mention of "Vivian Kubrick" is not also counted as a mention of "Kubrick" (the same rule applies to the Mentions view and to passive marking). Chapters beyond the greater of your X-Ray coverage and reading position are dimmed with tap-to-reveal spoiler protection (per chapter, or **"Reveal all chapters"** in one step); the chapter crossing your position counts only up to it. For complete X-Rays, all chapters are visible with no spoiler gating. **"All appearances"** opens the entry's full mention list. Tap a chapter with mentions to jump into the book and launch a text search for the item's name and aliases (regex OR for multi-term matching); a floating **"← X-Ray"** button brings you back to your reading position and the view you came from. Uses TOC-aware chapter boundaries with page-range fallback for books without TOC. On EPUBs the counts come from the book's **name index**: where each X-Ray name appears, found once in the background (a few seconds for a novel, up to a minute or two for a collected works on an e-reader) and kept next to the book, so the tree and the whole-book Mentions open at once afterwards. New names or aliases are found on their own, and a font or margin change finds them again for the new layout. If you open Chapter Appearances before the index is ready, it finds the names right then, with a message you can tap to cancel; a Section X-Ray opens on its own pages at once and reads the rest of the book the first time you reveal beyond them. PDFs search the book each time.
...
- **Search X-Ray**: find any entry across all categories by name, alias, or description. When the book has more than one X-Ray, a row at the bottom extends the search: **"Search this book's section X-Rays (N)"** from the main X-Ray, **"Search the main X-Ray"** or **"Search the main X-Ray and other sections (N)"** from inside a section, with results grouped by X-Ray. If the book is in a group, the other books' hits are already folded into the same list as **"From *Title*"** groups
- **Local X-Ray Lookup**: select text while reading → instantly look it up in cached X-Ray data. No AI call, no network, instant results. Searches by name and alias (not descriptions, to avoid false matches like "Swift" hitting unrelated entries). An exact name or alias match opens that entry directly; several entries sharing the name give a short chooser; partial matches always open a results list (a lone partial match is never opened for you), grouped by X-Ray when they span several. Smart fallback for a single X-Ray: prefers the section covering the current page, falls back to main, uses a sole section out of range, or shows a picker. If the book is in a [Book Group](#book-groups), a name the open book has no entry for is answered from its carried list and then from the group's other X-Rays. When a lookup finds nothing (or finds the wrong entries), the dialog offers **"Add as alias of an entry…"**: pick the entry the name belongs to and it is remembered for this book, so marking and lookup recognize it from then on. The same row sits at the top of the browser's search results. Available in highlight menu and dictionary popup when any X-Ray cache exists (main or section). See "Look up in X-Ray" in [Highlight Mode](#highlight-mode).
...
- **Passive marking**: words on the page that match an X-Ray entity's name or alias get a discreet dotted gray underline as you read (EPUB page mode; drawn shortly after the page settles, entirely on device). Tap a marked word to open its entry. Only the page you are on is read, so marking costs the same on any size of book. Density is configurable (every occurrence, once per page, only after 10 or 25 unseen pages, or first appearance only; default: after 10 unseen pages; the last three read the book's name index, and until it is ready a name is marked once per page), as is which categories are marked (everything, people only, or people + places). In a [Book Group](#book-groups) the marks also cover names from this book's carried list and from the group's other X-Rays the spoiler chain allows; they are drawn the same dotted way, since that content is already behind you.
...
**Choosing categories and depth:** a new X-Ray can track everything or a narrower pick: people, places, ideas, terms, events. Four presets sit above the per-category toggles: **All categories**, **Characters and story (people, timeline)**, **Reference (everything except the timeline; the default since v0.22.1)**, and **Characters only** — the last two are much cheaper on a long novel, since the timeline is the single heaviest block. A separate **depth** dial decides how much each entry carries: **Light** (one line per entry, recurring figures and turning points only), **Standard** (a few sentences per entry, everything the reader meets; the default), or **Deep** (longer entries, every figure and development, richer connections). The creation form carries a **"Checkpoint spacing, categories, depth:"** row whose buttons show the current picks and open their pickers whenever the pick starts a new lineage (create or rebuild); the defaults are set globally in **Settings → Reading & Library → X-Ray → Categories for New X-Rays** and **Depth of New X-Rays**, and per book in Book Settings → X-Ray. A plain extend keeps the categories and depth the X-Ray was built with.

Tapping X-Ray always opens its popup, whatever the coverage: it leads with **View** when a cache exists, and the Extend…/Rebuild… rows, version rows (install a checkpoint, switch to complete, switch back), section rows, merges, and "Marking & lookup…" sit under it. A book with only section X-Rays, or with X-Rays from other books in its group, gets the "Marking & lookup…" row too.
...
**Automatic X-Ray (opt-in):** KOAssistant can quietly keep a book's X-Ray up to your reading position in the background as you read, so you never face one giant catch-up extraction on a long book. It is **off by default** and only ever continues an X-Ray you started; there are two ways to opt in:

- **Per book:** set **Automatic X-Ray: On** for the book — from its X-Ray popup or [Book Settings](#book-settings). A per-book **On** is self-contained: it covers the first build too, no global setting needed. It never spends anything unannounced — turning it on names how many background requests it would take right now and waits for **Start**, and **Cancel** puts the switch back the way it was. A per-book **Off** always wins.
- **Globally:** enable **Settings → Reading & Library → X-Ray → Automatic X-Ray (all books)** to keep every book's X-Ray up to date the same way. This setting only *continues* X-Rays you have already started: a book with no X-Ray is left alone until you start one for it. Individual books can still override either way.

Nothing ever starts a book's X-Ray on its own. The two ways to start one are the creation form's **"In checkpoints, as I read (automatic)"** pick and the per-book **Automatic X-Ray: On** switch, and both name the spend and wait for **Start** before the first request. If a book is left switched On while it still has no X-Ray, the same confirmation appears the moment the first build would start, with **Cancel (turn Automatic X-Ray off)** as the way out.

Deleting a book's X-Ray also turns that book's Automatic X-Ray off, so nothing re-asks or restarts behind you after a delete.
...
**Book groups and series:** for a series or a project spanning several books, put the books in an ordered [Book Group](#book-groups). Later books can then fold in earlier books' X-Rays, carry entities forward (they wake automatically when they appear), and keep one naming canon across volumes; the X-Ray popup's and browser's merge rows, and the Group Hub's fold row, drive it. Lookups reach across the group too: a name the open book does not know yet is answered from the earlier volumes' X-Rays, labelled with the book it came from. See [Book Groups](#book-groups).
...
| `{user_input}` | Any | The text typed in the input dialog, placed where the placeholder sits (without it, typed text is appended after the prompt). Actions using it stay out of the tap-only menus (highlight menu, dictionary popup); if you run such an action without typing anything, the placeholder is left in the prompt as written, which is why these actions are kept off those menus | None |
...
| `{doi_clause}` | Book, Highlight (reading) | A sentence naming the book's DOI when one was detected, for [Research Mode](#research-mode) prompts; empty when there is none | None |
...
| `{quiz_instructions}` | The chapter-quiz instructions and the JSON shape the Quiz action asks for | **Conditional**: only filled for quiz requests, empty otherwise |
| `{highlight_analysis_nudge}` | An instruction to also report which passages you highlighted, in a fixed JSON shape | **Conditional**: only appears when highlights are present. No built-in action uses it any more; it is kept for custom actions |
...
**Update-prompt placeholders (incremental caches only):** `{cached_result}`, `{cached_progress}`, `{incremental_book_text}`, `{incremental_book_text_section}` and `{entity_index}` fill in only inside an action's `update_prompt`, the prompt used when an existing artifact is updated instead of regenerated. They carry the artifact you already have, how far it covers, and only the newly read text. In an ordinary `prompt` they stay as written.
...
Actions appear as "Explain (KOA)", "Translate (KOA)", etc. in the highlight popup. Long-press any action for its description plus **Remove from the highlight menu**, **Other placements…**, **Edit…**, **Duplicate as custom action…** and (for a modified built-in) **Reset to default**, the same menu you get on every surface an action can be triggered from.
...
- **For this book**: Domain is saved in the book's sidecar folder, in KOAssistant's own `koassistant_book_settings.lua` file. Every time you open this book, its domain is used automatically.
...
- **Per-book domains persist**: A domain set "For this book" is saved in KOAssistant's own per-book settings file next to the book and restored every time you open it, even from file browser or artifact views. Set "None" on the book target to explicitly opt a book out of your global domain.
...
**Book Settings** is a per-book configuration screen. Every setting here applies only to the book it's set on, sticks to that book (stored in KOAssistant's own settings file inside the book's `.sdr` folder, restored each time you open it), and overrides the corresponding global default. Settings you don't touch stay on **Follow global**.
...
| Setting | Options | Notes |
|---|---|---|
| **Automatic X-Ray** | Follow global / On / Off | One switch for background X-Ray upkeep on this book. **On** is self-contained: it covers the first build too, regardless of the global setting, and confirms how many background requests it needs before starting anything. **Off** always wins. See [Automatic X-Ray](#x-ray-auto-update). |
...
**Creating groups** (main menu → **Groups**, or hold the **Group** row in Book Settings or on the Book Hub; a book that is in no group opens the same popup on a plain tap):
...
**Storage System (v3)**: Chats are organized into three storage locations:
...
**Migration**: If you're upgrading from an older version, your existing chats and per-book settings are moved into the plugin's own sidecar files automatically on the first launch (out of KOReader's `metadata.lua`, which the plugin no longer writes to). Very old installs that still keep chats in hash folders are imported the same way; those old files are backed up to `koassistant_chats.backup/`. One side effect worth knowing: KOReader's own **Reset** on a book (file browser long-press → **Reset**) clears KOReader's reading position, highlights and settings, but no longer touches KOAssistant's chats, artifacts or per-book settings. They live in the plugin's own files in the same folder, and deleting the book still removes everything.
...
### Provider & Model
- **Model: <model> (<Provider>)**: one row for both. Tap it to open the provider and model hub (32 built-in providers + custom)
...
#### X-Ray
Background X-Ray settings keep a book's X-Ray current as you read (the automation itself is off by default; individual books can override either way in [Book Settings](#book-settings) or the X-Ray popup). Background runs extract book text and spend API tokens **without a per-request tap** — text-extraction consent (or a trusted provider) is still required, and instead of the interactive size warnings they use hard guards (an oversized background extraction aborts). See [Automatic X-Ray](#x-ray-auto-update).
- **Automatic X-Ray (all books)**: Keep the X-Rays you have started up to date as you read (flowing formats like EPUB only): [checkpoints](#x-ray-version-ladder) at chapter-sized steps, always keeping the next one ready ahead of you: reaching it installs it instantly and the one after starts building. Books with no X-Ray are left alone: start one from the X-Ray popup ("In checkpoints, as I read (automatic)"), or switch that book to Automatic individually, which works even with this off. Per-book On/Off overrides win in both directions, and every first build asks before it spends. Spend guards: a cooldown before retrying a step that was declined, failed or cancelled, WiFi only, text-extraction consent required. (default: OFF)
- **Cooldown (minutes)**: Minimum wait before retrying an automatic checkpoint after one was declined, failed or cancelled (default 15; 0 = no wait). A build that finishes does not wait, so a chain keeps going.
...
- **Provider Settings**:
  ...
  - **Gemini Content Filter**: Google's own filter can block answers about violent or sexual passages in a book, and the reply then comes back empty with the reason named. **Relaxed for books** (default) turns the four adjustable filter categories off for this plugin's requests; **Google default** sends no filter setting at all. The same row appears in the Gemini model menu as "Content filter: …".
- **AI Book Tools (Experimental)**: Let the AI search and read the open book on demand to ground its answers in the actual text. See [AI Book Tools](#ai-book-tools-experimental). Supported on Gemini, Claude (Anthropic), OpenAI (API or Subscription), OpenRouter (Claude/GPT/Gemini models), plus DeepSeek, Mistral, Groq, xAI, Fireworks, Qwen (`qwen3-max`), Kimi (`kimi-k2.6`), NVIDIA, OpenCode Zen and OpenCode Go (tool-capable models only), and Ollama; other providers fall back to normal chat. Requires "Allow Text Extraction".
  ...
  - **AI Book Tools: Read Short Texts Whole**: On by default. When the readable text (up to your position while spoiler protection is on) is short enough, the AI gets it in one piece instead of searching it: about 64,000 characters at Quick and Standard effort, 128,000 at Thorough (Arabic, Cyrillic or CJK text counts two to three bytes per character). A novella, a short story or the first chapters of a long book are then covered in full. Off: always search.
...
**Older backups:** if a backup was made before KOAssistant moved chats into the books' own sidecar files, restoring imports its chats into the current storage automatically, so they show up in Chat History without an extra step.
...
- **What it does**: gives the AI three local tools — `search_book` (word search: exact phrase first, then every word, then most words of a longer query; very common words of the book are not required; whole-word hits before hits inside longer words; 12 best hits per query with the exact total; spelling must match), `read_around` (read a page window around a hit or page), and `toc` (table of contents, filterable by level and title) — and lets it call them before answering. In a long EPUB the search uses KOReader's own document search (the one behind the reader's Search dialog) in a background process, so the screen stays live and a tap on Stop cancels; the progress window counts the seconds. Before the first lookup the AI sees a short outline of the contents and how far it may read. The search and extraction run **on your device**; only the AI's own tool requests and the bounded text excerpts it asks for are sent to the provider. Every result says what it could not reach (caps, spoiler limit, sections you hid), and the answer is asked to pass that on. When the readable text is short enough it is sent whole instead (see "Read Short Texts Whole" below).
...
- **Providers**: **Gemini**, **Claude (Anthropic)**, **OpenAI** (API or Subscription), and **OpenRouter**, plus **DeepSeek, Mistral, Groq, xAI, Fireworks, Qwen, Kimi, NVIDIA, OpenCode Zen, OpenCode Go** (and **Ollama** locally, where a local model's capabilities are derived once from the model menu, e.g. on pick or via "Refresh model capabilities", before the Tools chip lights up) on tool-capable models. A few models reached through OpenCode Go answer tools but refuse the forced-tool step, so they are left out. Providers/models without tool support automatically fall back to normal chat.
...
> **Automatic X-Ray upkeep (opt-in):** X-Ray can also **update in the background** as you read (a silent request path that keeps the cache close to your position without prompting), once you have started that book's X-Ray and confirmed its first build. Prior X-Ray states are archived to a rolling **version history** — browse, restore, or delete earlier versions (with a configurable keep-count), which is handy for re-reads. These are opt-in; see [Reading Analysis Actions](#reading-analysis-actions).
...
**Per-minute plan limits:** some providers (Groq, Cerebras, OpenAI) count the answer budget a request *asks for* against a per-minute token allowance before running it, so on a small plan even a short question can be refused. KOAssistant learns that allowance from the provider's own replies — its rate-limit headers, or the wording of a refusal — and sizes later requests to fit: a refusal that a smaller answer could have avoided is resent once with a budget the plan can admit, and after that requests are capped before they go out. (If the allowance is simply spent for this minute, nothing is resent — the error says to wait and try again.) **Test provider…** records the allowance too, so it is often known before your first real request. If the prompt alone is already larger than the plan (or than the model's context window), a brief notice says so and the request is still sent, in case the plan has changed. A refusal that a smaller answer cannot fix comes back with one tip naming the lever where you are: a smaller scope or a new chat while reading, attaching less in a general chat, fewer folders for a library action. A background X-Ray step stopped this way reports "request too large" as its reason.

When the provider says how long to wait, that number is passed on: the message reads "the provider asks for a wait of about N seconds", and a background X-Ray step waits that long before its one retry instead of guessing.

**Account limits are told apart from rate limits:** when the message is really about the account (credits used up, balance empty, a spending limit reached), it is never retried and never told to wait. The tip says it is an account limit at that provider, and that adding credits or raising the limit is the only fix. Providers whose wording is identical to an ordinary rate limit are told apart by the error code the reply carries, which is now shown in the message. A background X-Ray step stopped this way reports "account credits or billing" and does not keep trying.
...
| **Amazon Bedrock** | AWS-hosted OpenAI models via Converse; Bedrock API key required | Docs-based* | [console.aws.amazon.com/bedrock](https://console.aws.amazon.com/bedrock/) |
...
> **AI Book Tools (experimental)**: **Gemini**, **Claude (Anthropic)**, **OpenAI** (API or Subscription), and **OpenRouter**, plus **DeepSeek**, **Mistral**, **Groq**, **xAI**, **Fireworks**, **Qwen**, **Kimi**, **NVIDIA**, **OpenCode Zen**, **OpenCode Go**, and **Ollama** (local models with derived tool support), can use local tools to search and read the open book on demand, for grounded, page-referenced answers. Availability is controlled by a per-chat **Tools** chip; the underlying setting is a simple on/off toggle (off by default) for whether the chip starts ON, overridable per book or globally. See [AI Book Tools](#ai-book-tools-experimental).
...
Several built-in hosted providers (Cerebras, MiniMax, DeepInfra, Novita AI, Hyperbolic, Nebius AI Studio, Chutes, Featherless, Vercel AI Gateway) ship with only a tiny *seed* model list. Pick them in **Settings → Model: …** and add the key in **Settings → API Keys & Auth**, then:
...
**Hold a button to close everything**: a long press on the **Close** button (and the other buttons in its row) in any KOAssistant view closes that view, any dictionary windows under it, and the highlight menu, taking you straight back to the page in one gesture.
...
**Good metadata improves AI responses.** Use Calibre, Zotero, or similar tools to ensure correct titles and authors. The AI uses this metadata for context in Book Mode and when "Include book info" is enabled for highlight actions. A title, author, or series edited in KOReader's own **Book information** screen counts as the real one everywhere KOAssistant shows or sends it (artifact and notebook lists, book pickers, groups, the library scan, chat history), and editing it refreshes that book's entry right away.
...
Pasted keys are cleaned up on read: spaces, line breaks (including one in the middle of the key, which is how a key copied out of a wrapped text file usually fails), and invisible characters are dropped, so a key that was saved with them starts working without you re-entering it. Saving a key through Settings tells you how many stray characters it removed.

> **Tip:** Provider pickers (inside the **Model: … (Provider)** row) list only providers that actually have an API key configured (plus local/no-key providers such as Ollama). If a provider is missing from the list, add its key in `apikeys.lua` or via Settings → API Keys & Auth.

### No response / timeout
1. Check internet connection
2. Enable Debug Mode to see the actual error
3. Try Test Connection in settings
4. Be patient with very long requests: a request is given up to 15 minutes to answer before the plugin gives up on it, so a big X-Ray or full-document run on a slow model is not cut off early
5. If the provider stopped the answer for its own reason (a content filter, a recitation block, a refusal), the reply now says so by name instead of reporting a broken response. A partial answer is kept and marked as cut short, so it is never saved as if it were complete. For Gemini this points at Settings → Advanced → Provider Settings → **Gemini Content Filter**
