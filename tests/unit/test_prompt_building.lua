...
local function runMessageBuilderTests()
    ...
    TestRunner:test("surrounding_context_section includes label when present", function()
        ...
        TestRunner:assertContains(result, "Surrounding context from the document")
...
local function runGatingTests()
    ...
    print("\n--- ContextExtractor: Basic Stats Opt-Out Pattern ---")

    TestRunner:test("progress allowed when enable_basic_stats=nil (default)", function()
    ...
    TestRunner:test("progress allowed when enable_basic_stats=true", function()
        local extractor = createMockExtractor({ enable_basic_stats = true })
        local data = extractor:extractForAction({})
        TestRunner:assertEquals(data.reading_progress, "50%")
    end)

    TestRunner:test("progress blocked when enable_basic_stats=false", function()
        local extractor = createMockExtractor({ enable_basic_stats = false })
        local data = extractor:extractForAction({})
        TestRunner:assertEquals(data.reading_progress, "")
    end)

    TestRunner:test("chapter info allowed when enable_basic_stats=nil (default)", function()
        local extractor = createMockExtractor({})
        local data = extractor:extractForAction({})
    ...
    TestRunner:test("chapter info blocked when enable_basic_stats=false", function()
        local extractor = createMockExtractor({ enable_basic_stats = false })
        local data = extractor:extractForAction({})
...
local function runSurroundingContextTests()
    ...
    TestRunner:test("{surrounding_context_section} disappears when empty", function()
        ...
        TestRunner:assertNotContains(result, "Surrounding context from the document")
