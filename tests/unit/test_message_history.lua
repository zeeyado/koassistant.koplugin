...
local function runCreateResultTextTests()
    ...
    TestRunner:test("reasoning indicator skipped in dictionary windows, web search kept", function()
        local h = MessageHistory:new()
        h:addUserMessage("Q")
        h:addAssistantMessage("Definition", nil, "deep thinking")
        for _idx, view in ipairs({ "compact_view", "dictionary_view" }) do
            local result = h:createResultText("word", { features = { [view] = true } })
            TestRunner:assertNotContains(result, "Reasoning/Thinking", view .. ": no reasoning line")
            TestRunner:assertContains(result, "Definition", view .. ": answer shown")
        end
        TestRunner:assertEqual(#h:getReasoningEntries(), 1, "the content stays for Show Reasoning")
        local web = MessageHistory:new()
        web:addUserMessage("Q")
        web:addAssistantMessage("Definition", nil, "deep thinking", nil, true)
        local wres = web:createResultText("word", { features = { compact_view = true } })
        TestRunner:assertContains(wres, "Web search was used", "other indicators stay")
        TestRunner:assertNotContains(wres, "Reasoning/Thinking", "reasoning still skipped")
    end)
    ...
    TestRunner:test("book context label", function()
        ...
        -- Book context inserts the metadata string directly (no "Book: " prefix)
        local result = h:createResultText("Title: 1984\nAuthor: Orwell", config)
        TestRunner:assertContains(result, "Title: 1984")
    end)
