...
function ChatGPTViewer:init()
  ...
  table.insert(minimal_button_row2, {
    ...
    callback = function()
      ...
      -- The user's explicit toggle must survive the re-run: without this marker,
      -- handlePredefinedPrompt re-resolves per-book > global and a per-book
      -- dictionary-context override would silently undo a "Ctx: OFF" pick.
      new_config.features._dictionary_context_explicit = true
      -- Close viewer and re-execute
      UIManager:close(self)
      local Dialogs = require("koassistant_dialogs")
      Dialogs.executeDirectAction(
...
function ChatGPTViewer:onShow()
  ...
  if self._stream_carry_anchor then
    -- The carry exists only when the reader deliberately scrolled away
    -- during the stream, so it outranks the exchange landing (2026-09-07:
    -- from the second reply on, scroll_to_last_question was tested first and
    -- the reading position was discarded while its page break stayed in
    -- the layout). On a miss the exchange landing is the fallback when set;
    -- a first reply keeps its natural landing (top) — this branch also
    -- serves viewers without transcript markers, e.g. streamed artifact text.
    if self.scroll_text_w and not self:landOnStreamPosition()
        and self.scroll_to_last_question then
      self:scrollToLastQuestion()
    end
  elseif self.scroll_to_last_question then
  ...
  if self._message_history and self.configuration then
    ...
    for k, v in pairs(self.configuration) do
      if type(k) == "string" and k:match("^_rerun_") then
        ...
        -- metatables (class methods like getCombinedLanguages vanish)
        expanded_config[k] = v
    ...
    if expanded_config.features then
      ...
      -- Reset the streaming toggle itself: minimal-popup dispatch (and the
      -- translate/dictionary per-view streaming settings) force
